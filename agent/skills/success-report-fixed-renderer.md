...
```text
Required fixed PDF renderer for Success Program executive briefs. Version 1.1.1, schema 1.1: strict evidence/classification validation, Turkish or English draft labels, source references, proposed priorities and success criteria, actual two-page A4 validation, and a PDF-only output directory whose SHA256 manifest is returned in memory instead of written as a file.
```

## Instructions

Use only after success-report-evidence-gate returns READY_TO_RENDER. This skill is executable implementation, not permission to export restricted content.

Implementation: executive_brief.py version 1.1.1; schema 1.1.
Normalized UTF-8/LF Python source SHA256: d2b29e16f9902c914e0c8ec31ceedd3427624188dee6ba16f1603764c35dfdae

1. Discover the runtime's approved skill, scratch and generated-file locations. Do not hard-code a host directory or guess a sandbox download URL.
2. Prefer programmatic extraction of the fenced Python block from this loaded skill resource, using a resource path actually supplied by the runtime. Otherwise reproduce the supplied source exactly. Normalize CRLF to LF and trailing whitespace to exactly one final newline, then verify its SHA256 before execution. Do not rewrite the renderer, remove gates or invent a replacement implementation.
3. Build the documented JSON schema from the already-approved evidence. Use schema_version 1.1, locale tr unless requested otherwise, confirmed_date null if genuinely unknown, and explicit classification evidence for the customer content AND every included source. Synthetic customer data does not declassify a real internal source. Never write a customer payload or render when classification is protected/unknown.
4. Follow the module docstring for all fields and limits. Each action needs priority P1/P2/P3 and success_criterion. Source IDs are unique S1..S99. Every service needs both customer-context and verified service-source references. Use friendly source titles without filename extensions, paths or URLs in visible text; raw URI/internal notes must stay outside the customer payload where possible. Empty lists must be explicit. The local synthetic_example.json mentioned in the docstring is a validation fixture, not a required runtime file.
5. Check installed Python, reportlab, pypdf and an existing local Unicode font pair. Do not install packages, download fonts or contact external converters automatically. Missing runtime capabilities are a BLOCKED_RUNTIME result.
6. Run the supplied renderer against an explicit existing trusted output directory. Use a unique ASCII basename under 57 stem characters, for example sp-<short-customer-slug>-<date-or-undated>-<random8>-draft.pdf. Do not overwrite another run. The CLI form is python -B executive_brief.py INPUT.json --output-dir EXISTING_DIR --filename SAFE.pdf; the API render(payload, directory, filename) returns the verified manifest. The renderer publishes exactly one file, the PDF: it never writes a manifest or any other sidecar into that directory, so a host that attaches everything the directory contains can only attach the deliverable.
7. ContentTooLong means revise/summarize the structured input within the evidence, then retry at most once. Never lower font sizes, remove classification/source gates, drop a material required service or fabricate an artifact. Other failures must be reported explicitly; retain only artifacts from a successful renderer return.
8. Verify the returned manifest matches the actual PDF bytes and page count. The manifest exists only as the value render() returns and the single JSON line the CLI prints; version 1.1.1 writes no manifest file, so never look for one, recreate one or save one next to the PDF. Read it silently for your own check and keep its fields, hashes, byte sizes and page counts out of every user-visible message. The manifest is not MIP protection, semantic evidence verification or proof that a download completed. Do not register a partial file. The script's test suite passed on Windows; successful execution in the current harness must be established, not assumed.
9. Return the real PDF through the platform's native created-file attachment mechanism, and attach the PDF only. The manifest JSON is internal verification data that never touches disk: never attach, upload, print, quote or link it, and never surface scratch input files or any other intermediate artifact, because they clutter the CSAM's chat and make a finished delivery look unfinished. Do not expose scratch code/input files, invent publish_file tools, fabricate Markdown URLs, or present a plain filename as a download link. Say the PDF card is below only if the host returned an actual created-file attachment. Distinguish file generation/card presence from completed user download.
10. Keep source links and the internal review note separate in the authorized chat. Never archive, email, share, publish or downgrade labels from this skill. Archive is a separate explicitly authorized and validated integration, not the native file card.
